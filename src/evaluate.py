...
Reports RMSE (in Newtons) for both BiLSTM-only and BiLSTM + ELM.
...
def main():
    ...
    rows = []
    for tag, filename in TEST_FILES.items():
    ...
    print(f"{'test_set':>22s}   {'BiLSTM (N)':>12s}   {'+ELM (N)':>12s}")
    for _, r in df_out.iterrows():
        print(f"  {r['test_set']:>22s}   "
              f"{r['rmse_bilstm']:>10.2f}     {r['rmse_combined']:>10.2f}")
    print(f"  {'OVERALL':>22s}   "
          f"{df_out['rmse_bilstm'].mean():>10.2f}     "
          f"{df_out['rmse_combined'].mean():>10.2f}")
    print(f"\nSaved: {args.output}")
